...
function handleNavigation(pageId) {
    ...
    console.log(`LOG: Handling navigation to page: ${pageId}`);
...
export function setupSheetScrollBehavior() {
    let isSheetOpenOnScrollStart = false;
    const sheet = document.getElementById('custom-sheet-menu');

    window.addEventListener('scroll', () => {
        if (sheet && sheet.classList.contains('open')) {
            console.log("LOG: Scroll detected, closing sheet menu.");
            setSheetOpen(false);
        }
    }, { passive: true });
}
...
export function setupNavigation() {
    console.log("LOG: Setting up all navigation listeners.");
    ...
    if (titleElement) {
        titleElement.addEventListener('click', () => {
            ...
            if (currentPage !== 'index') {
                console.log("LOG: Title clicked, navigating to home.");
                loadPage('index');
            }
        });
    }
...
export function setupChangelogFilters() {
    ...
    const filterChangelog = (platform) => {
        console.log(`LOG: Filtering changelog by platform: ${platform}`);
...
export function setupThemeChooserEvents() {
    ...
    if (!dialog || !railButton || !mobileButton || !closeButton) {
        console.error("Um ou mais elementos do seletor de tema não foram encontrados.");
        return;
    }
...
export function setupLanguageChooserEvents() {
    ...
    if (!dialog || !railButton || !mobileButton || !closeButton) {
        console.error("Um ou mais elementos do seletor de idioma não foram encontrados.");
        return;
    }
